...
public class EqualThanExpression extends BinaryExpression implements Expression
{
    /**
    ...
    public EqualThanExpression(Expression expressionA, Expression expressionB, Expression errorRecoveryExpression)
    {
	...
	else if(valueA.getType() == DataType.BOOLEAN || valueB.getType() == DataType.BOOLEAN)
	    return new BooleanValue(valueA.getValueAsBoolean() == valueB.getValueAsBoolean());

	// Assume they are strings.
	else
	    return new BooleanValue(valueA.getValueAsString().equals(valueB.getValueAsString()));
    }
